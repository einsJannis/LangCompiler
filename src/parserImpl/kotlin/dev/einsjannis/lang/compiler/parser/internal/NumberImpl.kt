...
class NumberImpl(token: Token) : Number {
    ...
    override val bytes: ByteArray = bytesFromString(typeDef?.let { token.content.substring(0, token.content.length - it.length) } ?: token.content)

    override val returnType: ReturnType = ReturnTypeImpl((when (typeDef) {
        "b" -> Types.Byte
        "i" -> Types.Integer
        "L" -> Types.Long
        null -> Types.Integer
